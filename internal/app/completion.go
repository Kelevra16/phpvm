...
const powershellCompletion = `Register-ArgumentCompleter -Native -CommandName phpvm -ScriptBlock {
    ...
    $commands = @('ui','dashboard','init','open','use','install','info','status','trust','check','serve','pie','supported','ls','list','ls-remote','current','which','resolve','shell','uninstall','prune','verify','repair','doctor','clean','exec','matrix','alias','sync','lock','restore','composer','import','ini','profile','ext','logs','cache','bundle','self-update','completion','laragon','version','help')
    ...
    $subcommands = @{
        logs=@('path','show','tail','open','clear','doctor'); cache=@('dir','list','verify','clear'); bundle=@('create','import')
        ini=@('path','show','diff','reset','defaults','preset','get','set'); profile=@('ls','create','set','use')
        trust=@('project','status','revoke'); pie=@('setup','path')
        open=@('logs','ini','root')
